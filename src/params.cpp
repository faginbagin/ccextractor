...
void parse_parameters (int argc, char *argv[])
{
    ...
    for (int i=1; i<argc; i++)
    {
		...
		{
            use_gop_as_pts = -1; // Don't use even if we would want to
			continue;
		}
            ...
            strcmp (argv[i],"-pn")==0)
        {
            ...
            {
                ts_forced_program=atoi_hex (argv[i+1]);
        ...
        {
            screens_to_process=atoi_hex (argv[i+1]);
		...
		{
			levdistmincnt = atoi_hex(argv[i+1]);
			i++;
			continue;
		}
        if (strcmp (argv[i],"-levdistmaxpct")==0 && i<argc-1)
		{
			levdistmaxpct = atoi_hex(argv[i+1]);
			i++;
			continue;
		}
		...
		{
			datastreamtype = atoi_hex(argv[i+1]);			
			i++;
			continue;
		}		
        if (strcmp (argv[i],"-streamtype")==0 && i<argc-1)
		{
			forced_streamtype = atoi_hex(argv[i+1]);
			i++;
			continue;
		}		
		/* Teletext stuff */
        ...
        if (strcmp (argv[i],"-unixts")==0 && i<argc-1)
		{
            ...
            utc_refvalue = t;
			i++;
			noautotimeref= 1; // If set by user don't attempt to fix
			continue;
		}		
        ...
        if (strcmp (argv[i],"-udp")==0 && i<argc-1)
		{
			...
			if (colon)
			{
				...
				*colon = ':';
				...
				udpaddr = ntohl(((struct in_addr *)host->h_addr_list[0])->s_addr);
				udpport = atoi_hex(colon + 1);
			}
			else
			{
				udpaddr = INADDR_ANY;
